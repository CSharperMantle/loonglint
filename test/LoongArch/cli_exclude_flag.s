...
# EXCLUDE-INTEGER: {{.*}}.bin:<raw>:0x0: delete branch to next instruction [loongarch:control/branch-to-next]
# EXCLUDE-INTEGER-NEXT:   - 0x00000000  b 4
# EXCLUDE-INTEGER-EMPTY:
# EXCLUDE-INTEGER-NEXT: 1 finding(s)
# EXCLUDE-INTEGER-NEXT: 1 loongarch:control/branch-to-next
...
# EXCLUDE-CONTROL: {{.*}}.bin:<raw>:0x4: delete or replace non-canonical NOP instruction [loongarch:integer/nop]
# EXCLUDE-CONTROL-NEXT:   - 0x00000004  move $a0, $a0
# EXCLUDE-CONTROL-EMPTY:
# EXCLUDE-CONTROL-NEXT: 1 finding(s)
# EXCLUDE-CONTROL-NEXT: 1 loongarch:integer/nop
...
# EXCLUDE-UNION-NOT: [loongarch:control/
# EXCLUDE-UNION-NOT: [loongarch:integer/

# RUN: loonglint --input-format=raw --arch=loongarch64 -E '^loongarch:(control|integer)/' %t.bin | FileCheck %s --check-prefix=EXCLUDE-CATEGORIES
