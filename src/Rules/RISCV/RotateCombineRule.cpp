...
RotateCombineRule::RotateCombineRule(const RISCVSpec &RISCVAS) : RISCVAS(RISCVAS) {}

StringRef RotateCombineRule::getID() const {
    return "riscv:integer/rotate-combine";
}

StringRef RotateCombineRule::getDescription() const {
    return "fuse adjacent rotation pair";
}
...
bool RotateCombineRule::shouldRun(const Context &) const {
