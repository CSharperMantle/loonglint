...
BextRule::BextRule(const RISCVSpec &RISCVAS) : RISCVAS(RISCVAS) {}

StringRef BextRule::getID() const {
    return "riscv:integer/bext";
}

StringRef BextRule::getDescription() const {
    return "fuse 'srl/srli; andi 1' into 'bext/bexti'";
}

unsigned BextRule::getInstructionCount() const {
    return 2;
}

bool BextRule::shouldRun(const Context &) const {
    return RISCVAS.hasZbs();
}

std::optional<Rule::Match> BextRule::match(ArrayRef<Instruction> Instructions,
                                           const Context &) const {
    using namespace LowLevelInstMatcherDSL;

    assert(Instructions.size() == 2 && "integer/bext requires two instructions");
