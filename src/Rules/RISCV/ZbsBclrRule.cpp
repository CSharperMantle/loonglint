...
ZbsBclrRule::ZbsBclrRule(const RISCVSpec &RISCVAS) : RISCVAS(RISCVAS) {}

StringRef ZbsBclrRule::getID() const {
    return "riscv:integer/zbs-bclr";
}
...
bool ZbsBclrRule::shouldRun(const Context &) const {
    return RISCVAS.hasZbs();
}

std::optional<Rule::Match> ZbsBclrRule::match(ArrayRef<Instruction> Instructions,
                                              const Context &) const {
    using namespace LowLevelInstMatcherDSL;

    assert(Instructions.size() == 4 && "integer/zbs-bclr requires four instructions");
