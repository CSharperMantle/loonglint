...
ShiftAddRule::ShiftAddRule(const RISCVSpec &RISCVAS) : RISCVAS(RISCVAS) {}

StringRef ShiftAddRule::getID() const {
    return "riscv:integer/shift-add";
}

StringRef ShiftAddRule::getDescription() const {
    return "fuse 'slli; add' into 'shNadd'";
}

unsigned ShiftAddRule::getInstructionCount() const {
    return 2;
}

bool ShiftAddRule::shouldRun(const Context &) const {
    return RISCVAS.hasZba();
}

std::optional<Rule::Match> ShiftAddRule::match(ArrayRef<Instruction> Instructions,
                                               const Context &) const {
    using namespace LowLevelInstMatcherDSL;

    assert(Instructions.size() == 2 && "integer/shift-add requires two instructions");
